        ...
        try:
            ...
            for service_name in monitoring_info:
                ...
                if service_info["email-notification"]:
                    ...
                    datetime_version_of_pulse_time = datetime_utils.cast_regular_as_datetime(service_pulse_time)
                    ...
                    if seconds_diff > service_info["pulse-time-diffs-(secs)"]:
                        ...
                        for email in email_list:
                            mail.send_email(filled_html_file, email, "Problem with " + service_name)
                        problem += 1
            if current_time.hour == 0 and current_time.second <= 18 and problem == 0:
                with codecs.open("good_status.html", "r") as good_status_html_file:
                    good_status_html = good_status_html_file.read()
                with open("email_list.txt") as email_list_file:
                    email_list = email_list_file.read().split("\n")
                for email in email_list:
                    mail.send_email(good_status_html, email, "All systems operational")
            sleep(10)
        except:
            sleep(5)
            continue
